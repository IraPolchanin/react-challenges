...
export default function App() {
  ...
  */
      ...
      //               SOLUTION 1
  // const handleKey = e => {
  //   const targetKey = synthKeys.find(key => e.type === "mousedown" ? key.keyName === e.target.dataset.note : key.computerKey === e.key)

  //   if (targetKey && !e.repeat) {

  //     const updateKey = key => ({
  //       ...key,
  //       active: e.type === "keyup" ? false : true,
  //       keyPressed:
  //         e.type === "keydown" ? true
  //           : e.type === "keyup" ? false
  //             : key.keyPressed
  ...
  //     setSynthKeys(prev => prev.map(key => key === targetKey ? updateKey(key) : key))
  //   }
  // }
  ...
  const handleKey = e => {
    setSynthKeys(prev => prev.map(key => (key.keyName === e.target.dataset.note || key.computerKey === e.key)
      ? {
        ...key,
        active: e.type === "keydown" || e.type === "mousedown",
        keyPressed:
          e.type === "keydown" ? true
            : e.type === "keyup" ? false
              : key.keyPressed
